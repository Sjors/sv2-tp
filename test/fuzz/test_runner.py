...
def main():
    ...
    if not args.generate:
        ...
        if test_list_missing_corpus:
            ...
            logging.info("Please consider adding a fuzz corpus at https://github.com/Sjors/sv2-tp-qa-assets")
...
def merge_inputs(*, fuzz_pool, corpus, test_list, src_dir, fuzz_bin, merge_dirs):
    ...
    for t in test_list:
        args = [
            ...
            # size of the sv2-tp-qa-assets git repository and to cut the runtime
            # required to iterate over all fuzz inputs.
            '-shuffle=0',
            '-prefer_small=1',
            '-use_value_profile=0',
            # use_value_profile is enabled by oss-fuzz, but kept disabled here to
            # avoid unnecessarily bloating the sv2-tp-qa-assets git repository.
