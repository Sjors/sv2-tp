...
bool Sv2CipherState::DecryptWithAd(std::span<const std::byte> associated_data, std::span<std::byte> ciphertext, std::span<std::byte> plain)
{
...
bool Sv2CipherState::EncryptWithAd(std::span<const std::byte> associated_data, std::span<const std::byte> plain, std::span<std::byte> ciphertext)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());
...
bool Sv2CipherState::EncryptMessage(std::span<const std::byte> plain, std::span<std::byte> ciphertext)
{
    ...
    for (size_t i = 0; i < num_chunks; ++i) {
        ...
        std::span<std::byte> chunk = ciphertext.subspan(bytes_written, chunk_size);
        std::span<std::byte> chunk_plain = ciphertext.subspan(bytes_written, chunk_size - Poly1305::TAGLEN);
...
bool Sv2CipherState::DecryptMessage(std::span<std::byte> ciphertext, std::span<std::byte> plain)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());
    ...
    while (processed < ciphertext.size()) {
        ...
        std::span<std::byte> chunk_cipher = ciphertext.subspan(processed, chunk_size);
        std::span<std::byte> chunk_plain = plain.subspan(plain_position, chunk_size - Poly1305::TAGLEN);
...
void Sv2SymmetricState::MixHash(const std::span<const std::byte> input)
{
    m_hash_output = (HashWriter{} << m_hash_output << input).GetSHA256();
}

void Sv2SymmetricState::MixKey(const std::span<const std::byte> input_key_material)
{
    NoiseHash out0;
    NoiseHash out1;
...
void Sv2SymmetricState::HKDF2(const std::span<const std::byte> input_key_material, NoiseHash& out0, NoiseHash& out1)
{
    NoiseHash tmp_key;
...
bool Sv2SymmetricState::EncryptAndHash(std::span<const std::byte> plain, std::span<std::byte> ciphertext)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());
...
bool Sv2SymmetricState::DecryptAndHash(std::span<std::byte> ciphertext, std::span<std::byte> plain)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());
...
void Sv2HandshakeState::WriteMsgEphemeralPK(std::span<std::byte> msg)
{
...
void Sv2HandshakeState::ReadMsgEphemeralPK(std::span<std::byte> msg)
{
...
void Sv2HandshakeState::WriteMsgES(std::span<std::byte> msg)
{
...
bool Sv2HandshakeState::ReadMsgES(std::span<std::byte> msg)
{
...
bool Sv2Cipher::DecryptMessage(std::span<std::byte> ciphertext, std::span<std::byte> plain)
{
...
bool Sv2Cipher::EncryptMessage(std::span<const std::byte> input, std::span<std::byte> output)
{
