...
inline void LogPrintFormatInternal(std::source_location&& source_loc, BCLog::LogFlags flag, BCLog::Level level, util::ConstevalFormatString<sizeof...(Args)> fmt, const Args&... args)
{
...
// Log unconditionally. Be conservative when using functions that unconditionally log to debug.log!
...
// is passed. Callers should take care not to introduce disk-fill vulnerabilities when logging
// unconditionally. Additionally, users specifying -debug are assumed to be developers or power
// users who are aware that -debug may cause excessive disk usage due to logging.
