...
public:
    ...
    InteractResult Interact()
    {
        ...
        while (true) {
            ...
            if (!m_to_send.empty()) {
                ...
                std::span<const uint8_t> to_send = std::span{m_to_send}.first(n_bytes_to_send);
    ...
    void SendPacket(Sv2NetMsg msg)
    {
        ...
        std::span<std::byte> buffer_span{MakeWritableByteSpan(ciphertext)};
    ...
    void ReceiveMessage(Sv2NetMsg expected_msg)
    {
        ...
        std::span<std::byte> encrypted_payload{MakeWritableByteSpan(m_received).subspan(SV2_HEADER_ENCRYPTED_SIZE, expanded_size)};
        std::span<std::byte> payload = encrypted_payload.subspan(0, header.m_msg_len);
